...
### Remarks
Use **DCLPROP** when creating a property for a class that doesn't need to provide specialized Get or Set methods. The compiler will create the Get and Set methods and the field that holds the property value, which is usable only by the compiler within the property Get and Set methods. It is not available to the programmer.

A one-line **DclProp** declaration is a much simpler way to write a simple save-and-retrieve-value **BegProp** block. The following two declarations are equivalent:

```
    DclProp FirstName *string INZ("") Access( *public )
```
...
```
    DclProp FirstName *string INZ("") Access( *public )
    DclProp LastName *string INZ("") Access( *public )
