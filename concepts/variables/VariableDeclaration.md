...
- An object type, or class, from Encore RPG or another application, such as `Label` or `TextBox`.
...
- However, if you declare a local variable within a block, its scope is that                      block only. A block is a set of statements terminated by an:
...
<!-- Eduardo: this file is missing!  [Lifetime](.html) -->
